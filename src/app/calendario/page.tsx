...
import {
  addDoc, collection, onSnapshot, query, where, Timestamp, doc, updateDoc, deleteDoc, orderBy
...
import { ChevronLeft, ChevronRight, Plus, Trash2, PencilLine, X, LogOut } from 'lucide-react'
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-purple-100 to-indigo-200 dark:from-gray-800 dark:to-gray-900 text-gray-900 dark:text-white p-6">
      ...
      <div className="mx-auto max-w-6xl mb-6 flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <button
            ...
            className="ml-4 px-3 py-2 rounded-lg bg-indigo-600 hover:bg-indigo-500 text-sm font-semibold"
          >
            Hoy
          </button>
        </div>
        ...
        <button
          onClick={() => openCreate()}
          className="flex items-center gap-2 px-4 py-2 rounded-lg bg-emerald-600 hover:bg-emerald-500 font-semibold"
        >
        ...
        {weeks.map((week, wi) =>
          week.map((day, di) => {
            ...
            return (
              <div
                key={`${wi}-${di}`}
                className={`min-h-[120px] bg-neutral-900 p-2 relative ${
                  out ? 'opacity-30' : ''
                }`}
                onDoubleClick={() => openCreate(dateISO)}
              >
                <div className="flex items-center justify-between mb-1">
                  <span className={`text-sm ${today ? 'text-emerald-400 font-bold' : 'text-neutral-300'}`}>
                ...
                <div className="flex flex-col gap-1">
                  {dayEvents.map((ev) => (
                    <div
                      key={ev.id}
                      className={`group ${CATEGORY_STYLES[ev.category]} rounded-md px-2 py-1 text-xs flex items-center justify-between`}
                    >
                      <div className="truncate">
                        <span className="font-semibold">{ev.title}</span>
                        {ev.startTime ? (
                          <span className="opacity-90"> · {ev.startTime}{ev.endTime ? `–${ev.endTime}` : ''}</span>
                        ) : null}
                      </div>
      ...
      <div className="fixed bottom-6 left-6">
        <button
          onClick={() => router.push('/')}
          className="p-3 rounded-full bg-red-600 hover:bg-red-500 shadow-lg text-white"
          title="Salir"
        >
          <LogOut size={20} />
        </button>
...
function EventModal({
  ...
  const handleChange = (k: keyof CalendarEvent, v: string) => {
      ...
      <div className="w-full max-w-lg bg-neutral-950 text-white rounded-2xl shadow-2xl border border-neutral-800">
        ...
        <div className="px-5 py-4 space-y-4">
          <div>
            ...
            <input
              ...
              className="w-full rounded-lg bg-neutral-900 border border-neutral-800 px-3 py-2 outline-none focus:border-indigo-600"
            />
          </div>
        ...
        <div className="px-5 py-4 border-t border-neutral-800 flex items-center justify-end gap-3">
          ...
          <button
            disabled={!canSave}
            onClick={() => onSave(form)}
            className={`px-4 py-2 rounded-lg font-semibold ${canSave ? 'bg-indigo-600 hover:bg-indigo-500' : 'bg-neutral-700 cursor-not-allowed'}`}
          >
            Guardar
          </button>
        </div>
